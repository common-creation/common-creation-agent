...
6. **Slack統合**: `config/agent.yml`のSlack設定でボット機能を有効化
7. **Slackスレッド履歴取得**: 環境変数 `SLACK_LOOKUP_THREAD_HISTORY=1` でスレッドの過去メッセージをコンテキストに含める機能を有効化（詳細は下記）

### Slackスレッド履歴取得機能

パブリック/プライベートチャンネルのスレッド内でメンションされた際に、過去のメッセージ（最新50件）を取得してLLMのコンテキストに含めることができます。
...
```bash
export SLACK_LOOKUP_THREAD_HISTORY=1
```

または `.env` ファイルに追加します：

```env
SLACK_LOOKUP_THREAD_HISTORY=1
```

#### 動作仕様

- **対象**: パブリックチャンネル、プライベートチャンネルのみ
- **除外**: DM（Direct Message）、MPDM（Multi-Person Direct Message）
- **取得件数**: 最新50件まで
- **取得タイミング**: メンション時に毎回取得
- **機能制御**: 環境変数が `1` の場合のみ有効（デフォルト: 無効）
...
user1: 東京の天気は？
bot: 晴れです
user2: @bot それは明日も続きますか？  ← スレッド履歴から「東京の晴れ」を理解
bot: はい、明日も東京は晴れの予報です
```

環境変数が無効の場合、ボットは「それ」が何を指すか分からない可能性があります。
